require_dependency 'devise/confirmations_controller'

# **************************************************************************
# **************************************************************************

class Philia::ConfirmationsController < Devise::ConfirmationsController
  skip_before_action :authenticate_tenant!
  before_action      :set_confirmable, only: %i[update show]
  ...
  def log_action(action)
    unless logger.nil?
      logger.debug(
        "PHILIA >>>>> [confirm user] #{action} - #{@confirmable.email}"
      )
    end
  end

  # PHILIA: adaptation of Devise method for multitenanting
  # Sign in a user
