...
module Philia
  module Generators
    ...
    class InstallGenerator < Rails::Generators::Base
      desc 'Full installation of philia with devise'

      source_root File.expand_path('templates', __dir__)

      class_option :use_airbrake, type: :boolean, default: false,
                                  desc: 'Use this option to add airbrake exception handling capabilities'
      class_option :skip_recaptcha, type: :boolean, default: true,
                                    desc: 'Use this option to skip adding recaptcha for sign ups'
      class_option :skip_invite_member, type: :boolean, default: false,
                                        desc: 'Use this option to skip adding invite_member capabilities'
      class_option :skip_env_email_setup, type: :boolean, default: false,
                                          desc: 'Use this option to skip adding smtp email info to config/environments/*'
      class_option :org_email, type: :string, default: 'my-email@my-domain.com',
                               desc: 'define the organizational email from address'
      class_option :skip_devise_generators, type: :boolean, default: false,
                                            desc: 'skip execution of devise generators (if this has already been done previously)'
      ...
      def setup_initial_stuff
        ...
        template 'initializer.rb', 'config/initializers/philia.rb'

        gem 'recaptcha', require: 'recaptcha/rails' unless options.skip_recaptcha
        gem 'airbrake' if options.use_airbrake
      ...
      def setup_devise
        ...
        gsub_file 'app/models/user.rb', /,\s*$/, ', :confirmable, :trackable,'

        migrate_user_file = find_or_fail('db/migrate/[0-9]*_devise_create_users.rb')

        uncomment_lines(migrate_user_file, /confirm/)

        uncomment_lines(migrate_user_file, /Trackable/)
        uncomment_lines(migrate_user_file, /sign_in_count/)
        uncomment_lines(migrate_user_file, /current_sign_in_at/)
        uncomment_lines(migrate_user_file, /last_sign_in_at/)
        uncomment_lines(migrate_user_file, /current_sign_in_ip/)
        uncomment_lines(migrate_user_file, /last_sign_in_ip/)
        ...
        gsub_file 'config/initializers/devise.rb', /config.mailer_sender = '.+'/,
                  "config.mailer_sender = '#{options.org_email}'"
      end
      ...
      def setup_philia_member
        unless options.skip_invite_member
          ...
          inject_into_file 'app/controllers/members_controller.rb', before: 'end' do
            snippet_fill_member_ctlr
          end

          directory File.expand_path('../../../app/views/members', __dir__), 'app/views/members'
          directory File.expand_path('../../../app/views/devise/registrations', __dir__),
      ...
      def wrapup
        alert_color = :red
        say('-------------------------------------------------------------------------', alert_color)
        say('-   philia installation complete', alert_color)
        ...
        say('-   please run migrations: $ rails db:migrate', alert_color)
        say('-------------------------------------------------------------------------', alert_color)
      end
    ...
    #   end   # Devise
      ...
      def gem_find_or_fail(list)
        ...
        list.each do |gem_name|
          gem_msg = `bundle info #{gem_name}`
          next unless /Could not find gem/i =~ gem_msg
        ...
        if need_fail
          ...
          say('-   then retry rails g philia:install', alert_color)
          say('-------------------------------------------------------------------------', alert_color)
      ...
      def file_find_or_fail(filename)
        ...
        if user_file.blank?
          ...
          say('-   first run  $ rails g philia:install', alert_color)
          say('-   then retry $ rails g web_app_theme:philia', alert_color)
          say('-------------------------------------------------------------------------', alert_color)
